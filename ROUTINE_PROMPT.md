...
Run order:
1. Read SKILL.md (operational playbook) and config/runtime.json (risk caps, thresholds, signal window).
2. Check the signal window: signals are only generated 02:30–08:30 GMT. If outside this window, post the kill-switch message and stop.
...
5. Only score tickers after the latest hourly candle has fully closed.
6. For each ticker that passes filters, score across the 6 confluence categories. Issue BUY only when score ≥ effective threshold (4/6 default, 5/6 when VIX 20–25 or counter-trend) AND risk:reward ≥ 1:2 AND not within 3 trading days of earnings.
7. Cap output at 3 BUY signals (highest confluence wins ties).
8. Post the formatted signal block to Slack using the connector. Format: "{TICKER} : BUY" per line, followed by the separator. Use the channel specified in config/runtime.json under slack_channel_id.
9. Do not place trades. Do not call any write APIs other than the Slack post. Do not commit to the repo.
